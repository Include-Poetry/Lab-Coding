/*	Programa auxiliar en el análisis del conteo de vueltas en ruedas
	...
	Programado por Ricardo Velázquez Contreras el 10/06/2017
...
void Captura(string FileName){	// Función quue captura los datos desde la ruta FileName
...
void Conversion(bool todo){		// Convertimos los datos de la captura en la hora del ratón
	...
	for (i=AnoI; i <= AnoF; i++){	// Control de los años
		for (j = MesI; j <= MesF; j++){		// Control de los meses
			for (k = DiaI; k <= DiaF; k++){		// Control de los días
				...
				for (l = 0, hora = 16; l < HoraCero; l++, hora++){
					...
					if (!todo && captura[i][j][k][l]) cambio = true;	// Determinación de cambios
				}
...
void GuardarConteos(string carpeta){	// Guardamos los conteos
	...
	for (int i=AnoI; i <= AnoF; i++){	// Control del año
		for (int j = MesI; j <= MesF; j++){		// Control del mes
			for (int k = DiaI; k <= DiaF; k++){		// Control del día
				// Mensaje al usuario
				salida << "Año " << i << " mes " << j << " día " << k << endl;
